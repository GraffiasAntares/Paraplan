...
public:
    ...
    int calculateFitness(int generation = 0, int maxGenerations = 1) const {
            ...
            for (int d = 0; d < Config::NUM_DAYS; ++d) {
                ...
                if (slots.size() > 1) {
                    ...
                    for (size_t i = 1; i < slots.size(); ++i)
                        soft_penalty += (slots[i] - slots[i - 1] - 1);
                }
            }
        }
        ...
        for (int g = 0; g < Config::NUM_GROUPS; ++g) {
            ...
            for (auto v : per_day)
                soft_penalty += std::pow(v - mean, 2);
        }

        // Динамический вес мягких штрафов
        double progress = (double)generation / std::max(1, maxGenerations - 1);
        double dynamic_soft_weight = 0.5 + 2.0 * progress;

        return (int)(
            hard_conflicts * Config::weights.hard_conflict +
            soft_penalty * Config::weights.soft_gap * dynamic_soft_weight
        );
    }
